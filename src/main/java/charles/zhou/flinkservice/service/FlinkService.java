...
public class FlinkService {
	...
	public void startFlinkJob() throws Exception {
		...
		// 模拟大规模数据
		List<String> largeData = Arrays.asList("user1,user1@example.com", "user2,user2@example.com",
				"user3,user3@example.com", "user4,user4@example.com", "user5,user5@example.com");
		...
		DataStream<Tuple2<String, String>> userStream = inputStream
		...
		// 对用户名进行分组，模拟分布式计算
		DataStream<Tuple2<String, Integer>> groupedStream = userStream.map(tuple -> Tuple2.of(tuple.f0, 1))
				.keyBy(tuple -> tuple.f0).reduce(new ReduceFunction<Tuple2<String, Integer>>() {
					private static final long serialVersionUID = 1L;
		...
		// 3. 状态管理
		// 统计每个用户出现的次数，并使用状态管理
		DataStream<Tuple2<String, Integer>> statefulStream = userStream.keyBy(tuple -> tuple.f0)
